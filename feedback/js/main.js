...
$(function () {
    ...
    var ProcessForm = function (parameters) {
        ...
        this.init = function () {
            ...
            $(submitForm).find('.attachments').addClass('dropzone');
            this.dropzone = new Dropzone(submitForm.getElementsByClassName('dropzone')[0],{
    ...
    // disabled и enabled изображений для FormData
    ProcessForm.prototype.addImages = function (dataForm) {
        if (!this.existenceUploadsFile) {
            return;
        }
        var fileList = this.dropzone.files;
            ...
            if (fileList.length > 0) {
                ...
                for (var j = 0; j < count; j++) {
                    ...
                    dataForm.append('attachment[]', fileList[j], fileList[j].name);
